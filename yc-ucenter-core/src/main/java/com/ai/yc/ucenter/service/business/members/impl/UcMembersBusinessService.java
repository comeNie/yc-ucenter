...
public class UcMembersBusinessService extends UcBaseService implements IUcMembersBusinessService {
	...
	public UcMembersRegisterResponse insertMember(UcMembersRegisterRequest request) {
		...
		List<String> listValidator = beanValidator(request);
		if (listValidator != null && !listValidator.isEmpty()) {
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.FAIL_CODE,
					listValidator + "", null);
			return response;
		}
		// 验证 用户名电话邮箱必输一项
		if (!UcmembersValidators.validateUserAccount(request)) {
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.USERNAME_ERROR,
					"用户名、电话邮箱必输一项", null);
			return response;
		}
		// 用户名已经存在
		if (StringUtils.isNotBlank(request.getUsername())
			...
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.USERNAME_EXISTS,
					"用户名已存在", null);
			return response;
		...
		if (StringUtils.isNotBlank(request.getEmail()) && !UcmembersValidators.validateEmail(request.getEmail())) {
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.EMAIL_REGISTERED,
					"该邮箱已注册", null);
			return response;
		}
		...
		if (StringUtils.isNotBlank(request.getMobilephone())
			...
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.MOBILE_REGISTERED,
					"该手机号码已注册", null);
			return response;
		}
		...
		try {
			...
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.SUCCESS_CODE,
					"用户注册成功", responseDate);
		} catch (Exception e) {
			response = (UcMembersRegisterResponse) addResponse(response, true, RegResultCodeConstants.FAIL_CODE,
					"失败,生成验证码失败", null);
	...
	public UcMembersGetResponse getMember(UcMembersGetRequest request) {
		...
		List<String> listValidator = beanValidator(request);
		if (listValidator != null && !listValidator.isEmpty()) {
			response = (UcMembersGetResponse) addResponse(response, true,
		...
		if (list.size() > 0) {
			...
			if (("0").equals(ucMembers.getEnablestatus())) {
				...
				response = (UcMembersGetResponse) addResponse(response, true,
						Constants.GetUcMembersResultConstants.NO_ACTIV, "账户未激活", responseMap);
				return response;
			}
			UcMembersGetDate ucMembersGetDate = new UcMembersGetDate();
			BeanUtils.copyProperties(ucMembersGetDate, ucMembers);
			...
			response = (UcMembersGetResponse) addResponse(response, true,
					Constants.GetUcMembersResultConstants.SUCCESS_CODE, "成功", responseDate);
			return response;
		} else {
			response = (UcMembersGetResponse) addResponse(response, true,
	...
	public UcMembersResponse updateMobilephone(UcMembersEditMobileRequest request) {
		...
		if (res == UcMembersOperationServiceAtomImpl.RESULT_VALI_DIFFERENT
			...
			response = (UcMembersResponse) addResponse(response, true, EditMobileResultCodeConstants.FAIL_CODE, "验证码错误",
					null);
			return response;
		...
		if (!UcmembersValidators.validateMobilephone(request.getMobilephone())) {
			response = (UcMembersResponse) addResponse(response, true, EditMobileResultCodeConstants.EXISTS_ERROR,
		...
		if (resultCount > 0) {
			...
			response = (UcMembersResponse) addResponse(response, true, EditMobileResultCodeConstants.FAIL_CODE, "失败",
					null);
		}
		return response;
	}
	...
	public UcMembersResponse updateEmail(UcMembersEditEmailRequest request) {
		...
		if (resultCount > 0) {
			response = (UcMembersResponse) addResponse(response, true, EditMobileResultCodeConstants.SUCCESS_CODE, "成功",
					null);
		} else {
			response = (UcMembersResponse) addResponse(response, true, EditMobileResultCodeConstants.FAIL_CODE, "失败",
					null);
	...
	public UcMembersResponse updatePassword(UcMembersEditPassRequest request) {
		...
		List<String> listValidator = beanValidator(request);
		if (listValidator != null && !listValidator.isEmpty()) {
			response = (UcMembersResponse) addResponse(response, true, EditPassResultCodeConstants.FAIL_CODE,
					listValidator + "", null);
			return response;
		}
		...
		if (("1").equals(checke_mode)) {
			...
			if (!oldpassMD5.equals(ucMembers.getPassword())) {
				...
				response = (UcMembersResponse) addResponse(response, true, EditPassResultCodeConstants.OLDPASS_ERROR,
						"旧密码输入有误，修改失败", null);
				return response;
			} // 2.2 封装修改密码对象
			else {
				...
				if (resultCount > 0) {
					...
					response = (UcMembersResponse) addResponse(response, true, EditPassResultCodeConstants.SUCCESS_CODE,
							"修改成功", responseDate);
				} else {
					response = (UcMembersResponse) addResponse(response, true,
	...
	public UcMembersResponse ucCheckeEmail(UcMembersCheckEmailRequest request) {
		...
		List<String> listValidator = beanValidator(request);
		if (listValidator != null && !listValidator.isEmpty()) {
			response = (UcMembersResponse) addResponse(response, true, CheckEmailResultCodeConstants.FORMAT_ERROR,
					listValidator + "", null);
			return response;
		}
		...
		if (resultCount > 0) {
			response = (UcMembersResponse) addResponse(response, true, CheckEmailResultCodeConstants.EXIST_ERROR,
					"该邮箱已被注册", null);
		} else {
			response = (UcMembersResponse) addResponse(response, true, CheckEmailResultCodeConstants.SUCCESS_CODE, "成功",
					null);
		}
		return response;
	}
	...
	public UcMembersResponse ucCheckeMobilephone(UcMembersCheckeMobileRequest request) {
		...
		List<String> listValidator = beanValidator(request);
		if (listValidator != null && !listValidator.isEmpty()) {
			response = (UcMembersResponse) addResponse(response, true, CheckMobilResultCodeConstants.FORMAT_ERROR,
					listValidator + "", null);
			return response;
		}
		int resultCount = iUcMembersAtomService.checkMobilephone(request);
		if (resultCount > 0) {
			response = (UcMembersResponse) addResponse(response, true, CheckMobilResultCodeConstants.EXIST_ERROR,
					"该手机号码已被注册", null);
		} else {
			response = (UcMembersResponse) addResponse(response, true, CheckMobilResultCodeConstants.SUCCESS_CODE, "成功",
					null);
		}
		return response;
	}
	...
	public UcMembersResponse ucEditUserName(UcMembersEditUserNameRequest request) {
		...
		if (request.getUid() == null) {
			response = (UcMembersResponse) addResponse(response, true,
					EditUsernameResultCodeConstantsEnum.FORMAT_ERROR.getIndex(),
					EditUsernameResultCodeConstantsEnum.FORMAT_ERROR.getValue(), null);
			return response;
		}
		List<String> listValidator = beanValidator(request);
		if (listValidator != null && !listValidator.isEmpty()) {
		...
		if (listByUsername.size() > 0) {
			response = (UcMembersResponse) addResponse(response, true,
					EditUsernameResultCodeConstantsEnum.EXISTS_ERROR.getIndex(),
					EditUsernameResultCodeConstantsEnum.EXISTS_ERROR.getValue(), null);
			return response;
		}
		...
		if (resultCount > 0) {
			response = (UcMembersResponse) addResponse(response, true,
					MessageCodeConstantsEnum.MESSAGE_SUCCESS.getIndex(),
					MessageCodeConstantsEnum.MESSAGE_SUCCESS.getValue(), null);
		} else {
			response = (UcMembersResponse) addResponse(response, true,
					EditUsernameResultCodeConstantsEnum.UPDATE_ERROR.getIndex(),
	...
	public UcMembersResponse ucDelMember(UcMembersDelRequest request) {
		...
		if (request.getUid() == null) {
			response = (UcMembersResponse) addResponse(response, true,
					DelMemberResultCodeConstantsEnum.FORMAT_ERROR.getIndex(),
					DelMemberResultCodeConstantsEnum.FORMAT_ERROR.getValue(), null);
			return response;
		}
		...
		if (resultCount > 0) {
			...
			response = (UcMembersResponse) addResponse(response, true,
					DelMemberResultCodeConstantsEnum.DEL_ERROR.getIndex(),
